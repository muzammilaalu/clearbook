// SalesAttachmentUpload.jsx
// User ek Excel file upload karta hai jisme 2 columns hain:
//   invoice_ref  — ClearBooks invoice reference/number
//   file_path    — local file path (sirf naam — actual file alag upload hogi)
//
// Flow:
//   1. Excel parse karo → rows nikalo
//   2. Har row ke liye actual file select karne ka option do
//   3. File ko base64 mein convert karo
//   4. Backend ko bhejo → backend invoice_ref se invoice_id match karega

import { useState, useRef } from 'react';
import { Upload, FileUp, CheckCircle, XCircle, Paperclip, Trash2, Download } from 'lucide-react';
import * as XLSX from 'xlsx';
import { salesService } from '../services/api';

export default function SalesAttachmentUpload({ businessId, showNotification }) {
  const [rows,       setRows]       = useState([]);  // [{ invoice_ref, file_name, file, file_data_base64, status }]
  ...
  const handleExcelUpload = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      const parsed = data
        .filter(r => r['invoice_ref'] || r['Invoice Ref'] || r['invoice_id'])
        .map((r, i) => ({
          id:              i,
          invoice_ref:     (r['invoice_ref'] || r['Invoice Ref'] || r['invoice_id'] || '').toString().trim(),
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Update row statuses
      const errorMap = {};
      (result.errors || []).forEach(e => { errorMap[e.invoice_ref + '_' + e.file_name] = e.error; });
      ...
      setRows(prev => prev.map(r => r.status === 'uploading' ? { ...r, status: 'error', error: 'Upload failed' } : r));
    } finally {
      setUploading(false);
    }
  };

  // ── Download sample Excel mapping template ────────────────────────────
  const downloadTemplate = () => {
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.aoa_to_sheet([
      ['invoice_ref', 'file_name'],
      ['INV-001',     'invoice1.pdf'],
      ['INV-002',     'receipt.png'],
    ]);
    ws['!cols'] = [{ wch: 18 }, { wch: 22 }];
    // Style headers
    ['A1','B1'].forEach(addr => {
      if (ws[addr]) ws[addr].s = { font: { bold: true, color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: '3B82F6' } }, alignment: { horizontal: 'center' } };
    });
    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Attach files to invoices by invoice reference</p>
          </div>
        </div>
          ...
          <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
            {rows.map(row => (
              ...
              >
                ...
                {/* Invoice ref */}
                <span className="font-mono text-xs bg-white border border-gray-200 px-2 py-0.5 rounded text-gray-700 w-28 flex-shrink-0 truncate">
                  {row.invoice_ref}
                </span>

                {/* File name */}
                <span className="text-gray-600 flex-1 truncate text-xs">
                  {row.file_name || <span className="text-gray-400 italic">No file</span>}
                </span>
      ...
      {rows.length > 0 && (
        ...
        >
          {uploading ? (
            ...
            <>
              <Upload size={18} />
              Upload {readyCount} Attachment{readyCount !== 1 ? 's' : ''}
            </>
          )}
        </button>
      )}
      ...
      {results && (
        <div className={`mt-4 px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2 ${
          results.failed === 0 ? 'bg-green-50 border border-green-200 text-green-800' : 'bg-amber-50 border border-amber-200 text-amber-800'
        }`}>
