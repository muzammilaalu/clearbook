    ...
    const handleUpload = async () => {
        ...
        try {
            const payload = readyRows.map(r => ({
                ...
                file_name: r.file_name || att_name || attachment_name,
        ...
        <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-5 flex gap-2">
                ...
                <p className="text-xs text-amber-800">
                    Enter the invoice <strong>ID number</strong> in the <strong>invoice_ref</strong> column (e.g., <code>1</code>, <code>2</code>).
                    Invoice IDs can be obtained from the Sales Attachments export.
                </p>
            </div>
                ...
                <div className="mb-5">
                    ...
                    <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                        {rows.map(row => (
                            <div
                                key={row.id}
                                className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-sm transition-all ${row.status === 'success' ? 'bg-green-50 border-green-200' :
            ...
            {/* Results */}
            {results && (
                <div className={`mt-4 px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2 ${results.failed === 0
